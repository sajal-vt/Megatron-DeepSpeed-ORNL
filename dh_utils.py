...
def master_from_host(host):
    ...
    master_addr = socket.gethostbyname(host)
...
def create_launch_command(prefix, params, job_id, job_nodes=None, DEEPHYPER_LOG_DIR="."):
    ...
    DEEPSPEED_ARGS = f"--deepspeed --deepspeed_config {ds_config_file} --deepspeed-activation-checkpointing --checkpoint-activations --use-flash-attn"
    python_exe = "python"
    ...
    params = GPT_ARGS + " " + OUTPUT_ARGS + f" --data-path {DATA_PATH}" + " " + DEEPSPEED_ARGS + f" --master-addr {job_master}"
